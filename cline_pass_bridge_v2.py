...
def _models_payload():
    """Synthetic /v1/models list (upstream /models 404s). Built from providers.json + curated catalog."""
    models = []
    seen = set()
    try:
    ...
    # Curated ClinePass catalog (all known models)
    # ONLY models that work through the ClinePass subscription.
    # Non-ClinePass models (poolside, zai, stepfun bare) are NOT included —
    # they get "model not found" or "insufficient_credits" from Cline's API.
    # The :free suffix is a display label only; stripped before upstream call.
    curated = (
        # Premium / subscription models (all covered by ClinePass sub)
        "cline-pass/kimi-k3",
        "cline-pass/glm-5.2",
        "cline-pass/glm-5.3",
        ...
        # Free tier label for deepseek-v4-flash (same model as premium,
        # but shown as :free so users know it's the cheap delegation option)
        "cline-pass/deepseek-v4-flash",
        # DeepSeek V4 Flash Vision (experimental) — same family as flash but
        # with image input. NOTE: upstream only resolves the BARE slug
        # "deepseek/deepseek-v4-flash-vision-exp"; the cline-pass/ prefix
        # returns "model not found" from api.cline.bot (verified 2026-08-21).
        "deepseek/deepseek-v4-flash-vision-exp",
        # Free Poolside model (OpenRouter, genuinely free — verified 2026-08-28)
        "poolside/laguna-s-2.1:free",
        # Windows bridge sync 2026-09-13 (this-device): new models added to
        # the catalog per user request. cline-free/* upstream is 403-walled for
        # non-Cline clients (verified 2026-09-10) — kept for picker visibility
        # until a cline-free account lands in providers.json.
        "cline-pass/deepseek-v4.1-flash",
        "cline-free/muse-spark-1.3-contributor",
        # Kali fleet sync 2026-09-18: match Windows 26-model catalog
        # (Drive guide "Catalog sync" line). Free-tier slugs serve on ANY
        # account (no sub needed) now that product-surface headers are in.
...
def _gen_cline_ua():
    """Return the Cline product-surface User-Agent string.

    Matches the Windows bridge v2.0 spec (SHARED guide, 2026-09-12):
    api.cline.bot gates cline-free/* models behind product-surface headers;
    X-CLIENT-TYPE: cline-sdk is the critical identifier.
    """
    return "Cline/3.0.39"
...
def _open_upstream(path, raw_body, token):
    req = urllib.request.Request(
        ...
        headers={
            ...
            "Accept": "*/*",
            "User-Agent": _gen_cline_ua(),
            "X-Title": "Cline",
            "HTTP-Referer": "https://cline.bot",
            "X-CLIENT-TYPE": "cline-sdk",
            "X-CLIENT-VERSION": "3.0.39",
            "X-PLATFORM": "cli",
            "X-PLATFORM-VERSION": "3.0.39",
            "X-CORE-VERSION": "3.0.39",
            "X-IS-MULTIROOT": "false",
            "X-Task-ID": "hermes-bridge",
        },
        method="POST",
    )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if is_sse:
            ...
            body = resp.read()
            # Unwrap Cline API envelope: {"data": {...}, "success": true} -> standard OpenAI
            try:
                envelope = json.loads(body)
                if isinstance(envelope, dict) and "data" in envelope and "success" in envelope:
                    body = json.dumps(envelope["data"]).encode()
                    log("unwrapped data envelope for OpenAI compat")
            except (json.JSONDecodeError, TypeError):
                pass  # not JSON or not an envelope — pass through as-is
            self.send_header("Content-Length", str(len(body)))
            self.end_headers()
            self.wfile.write(body)
